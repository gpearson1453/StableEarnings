...
def splitText(full_text):
    """
    Split the full text of a file into segments based on the 'All Rights Reserved.' marker. 
    Returns a list of cleaned segments with leading and trailing whitespace removed.
...
def apply_stupid_horse_name_fixes(folder_path):
    ...
    and move replaced names to the 'fixed' dictionary. The changes are saved to a JSON file.
    ...
    for file_name in os.listdir(folder_path):
        if file_name.endswith(".txt") and file_name in stupid_horse_names:
            ...
            # Apply replacements for the current file
            replacements_made = []
            ...
            # Move the replaced names to the 'fixed' dictionary
            if file_name not in fixed:
                fixed[file_name] = []
            fixed[file_name].extend(replacements_made)

            # Remove the replaced names from stupid_horse_names and save the changes
...
def process_single_file(file_path):
    ...
    and extracting relevant race data from each segment using the getRaces function.
...
def process_files(folder_path, output_csv_file):
    """
    Process all text files in the specified directory, extract data, and write the results to a CSV file.

    Parameters:
    - folder_path (str): The path of the directory containing the text files.
    - output_csv_file (str): The name of the output CSV file to save the extracted data.
    """
