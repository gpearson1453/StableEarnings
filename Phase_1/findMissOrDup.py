...
in select_race_data.xlsx, the Excel file generated by processSelectToExcel.py, after processSelectToExcel has been run with
that folder. It searches for missing files by extracting dates and locations from the provided text block, which must be
manually created using text on Equibase for that month, and looking through the Excel file to ensure that those values
appear. Finally, it prints out the information of missing and duplicate files, if any are found.
...
Steps:
    ...
    - Verifies the presence of specific date and track combinations in the dataset using a text block containing all race
      locations and dates for a certain month.
...
Functions:
    ...
    - extract_date_from_text: Extracts a date in 'Month Day, Year' format from a text block.
    - check_tracks_in_excel: Verifies that track names in the text blocks are present in the dataset for a given date.
    - main: Coordinates the loading, validation, and duplicate checking processes.

Usage:
    Execute the script directly to load the dataset, process date-track combinations, and identify duplicates.
    Ensure the 'file_path' and 'date_track_blocks' are correctly defined before running.
"""
