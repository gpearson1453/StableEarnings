...
in select_race_data.xlsx, the Excel file generated by processSelectToExcel.py, after the script has been run with that
folder. It searches for missing files by extracting dates and locations from the provided text block, which must be manually
created using text on Equibase for that month, and looking through the Excel file to ensure that those values appear.
Finally, it prints out the information of missing and duplicate files, if any are found.
