...
def get_last_file_number(csv_file):
    ...
    print(f"Opening CSV file: {csv_file} to find the last file number.")
    with open(csv_file, 'r', encoding='utf-8') as f:
        reader = csv.reader(f)
        rows = list(reader)
        last_row = rows[-1]
        print(f"Last file number found: {last_row[0]}")
...
def find_target_folder(pdf_name):
    ...
    print(f"Extracting date and track information from PDF name: {pdf_name}")
    ...
    print(f"Month: {month}, Year: {year}, Month Number: {month_number}")
...
def get_missing_or_max_number(folder, starting_number):
    ...
    print(f"Looking for missing or max number in folder: {folder}")
    ...
    print(f"Existing file numbers: {existing_numbers}")
    ...
    print(f"No missing numbers, using next available number: {next_number}")
    return next_number
...
def save_text_to_file(folder, file_number, folder_name, text):
    ...
    print(f"Saving text to file: {text_file_path}")
    try:
...
def process_missing_pdfs(pdfs_folder, text_folder_base, csv_file):
    ...
    print(f"Starting file number is: {current_number}")
    ...
    for pdf_file in os.listdir(pdfs_folder):
        ...
        print(f"Next file number will be: {current_number}")
