...
class RacePredictor(nn.Module):
    def __init__(self, input_dim, embed_dim, w_embed_dim, ts_embed_dim, num_heads, output_dim=3):
        super(RacePredictor, self).__init__()
        self.embedding = nn.Linear(input_dim, embed_dim)  # Transform raw features into embeddings
        self.weather_embedding = nn.Embedding(8, w_embed_dim, padding_idx=0)  # weather embedding
        self.track_state_embedding = nn.Embedding(12, ts_embed_dim, padding_idx=0)  # track_state_embedding
...
def train_model(model, batches, num_epochs=10, learning_rate=0.001):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in batches:
            ...
            weather = batch[:, :, 41:42]  # Weather as categorical
            track_state = batch[:, :, 43:44]  # Weather as categorical
            targets = batch[:, :, -3:]  # Labels
            
            targets = torch.tensor(
                np.array([df.iloc[:, -3:].to_numpy(dtype=float) for df in batch]), dtype=torch.float
            )  # this is just to stop the code and throw an error
...
if __name__ == "__main__":
    ...
    model = RacePredictor(input_dim, embed_dim, w_embed_dim, ts_embed_dim, num_heads)
